...
select * from tmp_dir where file_link not in (select file_link from downloads) and file_link not in (select file_link from magfiles) and file_link not in (select file_link from labelfiles) and file_link not in (select file_link from extras where file_link is not null) and file_link not in (select file_link from files where file_link is not null) and file_link not in (select file_link from scraps where file_link is not null) order by file_link;
...
update files d inner join tmp_dir t on d.file_link = t.file_link set d.file_size = t.file_size where d.file_link like '/zxdb/%';
