...
select concat('insert into tags(id, name, tagtype_id, link) values ((select k from (select max(id)+1 as k from tags where tagtype_id=''C'') as x), ''',y.name,''', ''C'', ',if(y.website<>'',concat('''',y.website,''''),'null'),');') as qry
from zxdb.entries e
...
where y.name not in (select name from zxdb.tags)
group by y.name
order by y.name;
...
from (
select replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(c.name,'CAFePARTY 2019 Invitations','Invitation'),' Tiny Intro (256b)',' 256b Intro'),' Intro 256B',' 256b Intro'),' Intro 4KB',' 4K Intro'),' Byte ','b '),'Kb ','K '),'kb ','K '),'1k','1K'),'4k','4K'),'8-bit - ','8Bit '),'8-bit ','8Bit '),'8bit','8Bit'),' plattform ',' Platform '),' intro',' Intro'),'Oldskool demo','Oldskool Demo'),'(normal results)',''),'(alternative results)','') as category
from zxdb.entries e
inner join zxdb.webrefs w on w.entry_id = e.id and w.website_id = 48
inner join public.productions_production p on p.id = replace(replace(w.link,'https://demozoo.org/productions/',''),'/','')
inner join public.parties_competitionplacing n on p.id = n.production_id
...
select concat('(',e.id,',',t.id,',',g.id,',',n.position,',''',(case when c.name like '%(normal results)' then 'normal results' when c.name like '%(alternative results)' then 'alternative results' else '*' end),'''),') 
as 'insert into members(entry_id, tag_id, category_id, member_seq, variant) values'
from zxdb.entries e
inner join zxdb.webrefs w on w.entry_id = e.id and w.website_id = 48
inner join public.productions_production p on p.id = replace(replace(w.link,'https://demozoo.org/productions/',''),'/','')
inner join public.parties_competitionplacing n on p.id = n.production_id
