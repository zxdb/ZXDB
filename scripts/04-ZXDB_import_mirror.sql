...
select * from extras where file_link like '/pub/%' and file_link not in (select file_link from tmp_mirror) order by file_link;

select * from tmp_mirror where file_link not in (select file_link from downloads) and file_link not in (select file_link from files) and file_link not in (select file_link from scraps where file_link is not null) and file_link not in (select file_link from extras where file_link is not null) and file_link not like '/pub/sinclair/magazines/%' and file_link not like '/pub/sinclair/%/' order by file_link;
