...
from (
select replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(c.name,'CAFePARTY 2019 Invitations','Invitation'),'Old School Demo','Oldschool Demo'),'Combined - Game','Combined Game'),' Tiny Intro (256b)',' 256b Intro'),' Intro 256B',' 256b Intro'),' Intro 4KB',' 4K Intro'),' Intro 4Kb',' 4K Intro'),' Intro 4k',' 4K Intro'),'640k ','640K '),' Byte ','b '),'Kb ','K '),'kb ','K '),'1k','1K'),'4k','4K'),'8-bit - ','8Bit '),'8-bit ','8Bit '),'8bit','8Bit'),' plattform ',' Platform '),' showcase',' Showcase'),' intro',' Intro'),' demo',' Demo'),' game',' Game'),'(normal results)',''),'(alternative results)','') as category
from zxdb.entries e
inner join zxdb.webrefs w on w.entry_id = e.id and w.website_id = 48
inner join public.productions_production p on p.id = replace(replace(w.link,'https://demozoo.org/productions/',''),'/','')
inner join public.parties_competitionplacing n on p.id = n.production_id
...
inner join zxdb.categories g on g.text = replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(c.name,'CAFePARTY 2019 Invitations','Invitation'),'Old School Demo','Oldschool Demo'),'Combined - Game','Combined Game'),' Tiny Intro (256b)',' 256b Intro'),' Intro 256B',' 256b Intro'),' Intro 4KB',' 4K Intro'),' Intro 4Kb',' 4K Intro'),' Intro 4k',' 4K Intro'),'640k ','640K '),' Byte ','b '),'Kb ','K '),'kb ','K '),'1k','1K'),'4k','4K'),'8-bit - ','8Bit '),'8-bit ','8Bit '),'8bit','8Bit'),' plattform ',' Platform '),' showcase',' Showcase'),' intro',' Intro'),' demo',' Demo'),' game',' Game'),'(normal results)',''),'(alternative results)','')
