...
select * from (
    ...
    union all
        select id,title,mag_issue,'title apparently mismatches magazine issue' from (select e.id, e.title, coalesce(i.number,'?') as number, coalesce(i.volume,'?') as volume, coalesce(i.date_year,'?') as date_year, coalesce(i.date_month,'?') as date_month, i.magazine_id, replace(replace(replace(replace(replace(replace(replace(m.name,'ACE (Advanced Computer Entertainment)','ACE'),'Sinclair User Club/DE','Sinclair User Club'),'Your Computer/ES','Your Computer'),'Spectrum Computing/ES','Spectrum Computing'),'Spectrum Computing/NO','Spectrum Computing'),'16-48 Magazine','16/48 Magazine Tape'),'C&VG (Computer & Video Games)','C&VG') as mag_name, concat(m.name,' #',coalesce(i.number,'?'),' ',coalesce(i.date_year,'?'),'/',coalesce(i.date_month,'?')) as mag_issue from entries e inner join issues i on i.id = e.issue_id inner join magazines m on i.magazine_id = m.id) as x where title not like '%DigiTape%' and magazine_id not in (267,323,348) and id <> 13559 and not (title = mag_name or title = CONCAT(mag_name,' ',number) or title = CONCAT(mag_name,' ',lpad(number,2,'0')) or title = CONCAT(mag_name,' No ',number) or title = CONCAT(mag_name,' No ',lpad(number,2,'0')) or lower(title) = lower(CONCAT(mag_name,' Nr ',lpad(number,2,'0'))) or lower(title) = lower(CONCAT(mag_name,' Nr ',number)) or lower(title) = lower(CONCAT(mag_name,' issue ',number)) or lower(title) = lower(CONCAT(mag_name,' issue ',lpad(number,2,'0'))) or lower(title) = lower(CONCAT(mag_name,' issue ',lpad(number,3,'0'))) or title = CONCAT(mag_name,' issue ',lpad(number,2,'0'),'a') or title = CONCAT(mag_name,' issue ',lpad(number,2,'0'),'b') or title = CONCAT(mag_name,' issue ',number,' [1]') or title = CONCAT(mag_name,' issue ',number,' [2]') or title like CONCAT(mag_name,' issue ',lpad(number,2,'0'),':%') or title like CONCAT(mag_name,' issue ',number,':%') or title like CONCAT(mag_name,' issue ',lpad(number,2,'0'),' -%') or title like CONCAT(mag_name,' issue #',number,' -%') or title like CONCAT(mag_name,' Vol ',volume,' No ',number) or title like CONCAT(mag_name,' issue ',date_year-1900,lpad(date_month,2,'0')) or title like CONCAT(mag_name,' issue ',date_year,'-',lpad(date_month,2,'0')))
    union all
...
r.link like concat(w.link,'%') or (r.website_id=10 and r.link like 'https://%.wikipedia.org/wiki/%') or (r.website_id in (16,19) and r.link like 'https://youtu.be/%') or (r.website_id in (16,19) and r.link like 'https://www.youtube.com/%') or (r.website_id=31 and r.link like 'https://%.itch.io/%'))
    ...
    union all
        select e.id,e.title,t.text,'** conflicting information about original publication' from entries e left join publicationtypes t on t.id = e.publicationtype_id where (e.publicationtype_id is null or e.publicationtype_id not in ('C','E','M')) and e.id in (select entry_id from compilations where is_original = 1)
    union all
