...
select * from (
    ...
    union all
        select e.id,e.title,d.file_link,'running screen does not follow filename convention' from entries e inner join downloads d on d.entry_id = e.id where d.filetype_id = 2 and d.file_link like '/zxdb/%' and d.file_link not like '%-run-%' and d.entry_id > 30000 and d.entry_id <> 37752
    union all
    ...
    union all
        select id,title,mag_issue,'title apparently mismatches magazine issue' from (select e.id, e.title, coalesce(i.number,'?') as number, coalesce(i.volume,'?') as volume, coalesce(i.date_year,'?') as date_year, coalesce(i.date_month,'?') as date_month, i.magazine_id, replace(replace(replace(replace(replace(replace(replace(m.name,'ACE (Advanced Computer Entertainment)','ACE'),'Sinclair User Club/DE','Sinclair User Club'),'Your Computer/ES','Your Computer'),'Spectrum Computing/ES','Spectrum Computing'),'Spectrum Computing/NO','Spectrum Computing'),'16-48 Magazine','16/48 Magazine Tape'),'C&VG (Computer & Video Games)','C&VG') as mag_name, concat(m.name,' #',coalesce(i.number,'?'),' ',coalesce(i.date_year,'?'),'/',coalesce(i.date_month,'?')) as mag_issue from entries e inner join issues i on i.id = e.issue_id inner join magazines m on i.magazine_id = m.id) as x where title not like '%DigiTape%' and magazine_id not in (323,348) and id <> 13559 and not (title = mag_name or title = CONCAT(mag_name,' ',number) or title = CONCAT(mag_name,' ',lpad(number,2,'0')) or title = CONCAT(mag_name,' No ',number) or title = CONCAT(mag_name,' No ',lpad(number,2,'0')) or lower(title) = lower(CONCAT(mag_name,' Nr ',lpad(number,2,'0'))) or lower(title) = lower(CONCAT(mag_name,' Nr ',number)) or lower(title) = lower(CONCAT(mag_name,' issue ',number)) or lower(title) = lower(CONCAT(mag_name,' issue ',lpad(number,2,'0'))) or lower(title) = lower(CONCAT(mag_name,' issue ',lpad(number,3,'0'))) or title = CONCAT(mag_name,' issue ',lpad(number,2,'0'),'a') or title = CONCAT(mag_name,' issue ',lpad(number,2,'0'),'b') or title = CONCAT(mag_name,' issue ',number,' [1]') or title = CONCAT(mag_name,' issue ',number,' [2]') or title like CONCAT(mag_name,' issue ',lpad(number,2,'0'),':%') or title like CONCAT(mag_name,' issue ',number,':%') or title like CONCAT(mag_name,' issue ',lpad(number,2,'0'),' -%') or title like CONCAT(mag_name,' issue #',number,' -%') or title like CONCAT(mag_name,' Vol ',volume,' No ',number) or title like CONCAT(mag_name,' issue ',date_year-1900,lpad(date_month,2,'0')) or title like CONCAT(mag_name,' issue ',date_year,'-',lpad(date_month,2,'0')))
    union all
    ...
    union all
        select e.id,e.title,g.name,'** CSSCGC title missing from compilation' from entries e inner join members m on m.entry_id = e.id inner join tags g on g.id = m.tag_id and g.name like 'CSSCGC Crap Games Contest%' left join entries k on k.title like 'CSSCGC Crap Games Competition%' and e.id <> k.id left join compilations c on c.compilation_id = k.id and c.entry_id = e.id where k.title = concat('CSSCGC Crap Games Competition',right(g.name,5)) and c.entry_id is null
    union all
        select e.id,e.title,k.title,'CSSCGC title missing from contest' from entries e inner join compilations c on c.entry_id = e.id inner join entries k on k.id = c.compilation_id and k.title like 'CSSCGC Crap Games Competition%' left join tags g on g.name like 'CSSCGC Crap Games Contest%' and g.name = concat('CSSCGC Crap Games Contest',right(k.title,5)) left join members m on g.id = m.tag_id and m.entry_id = e.id where m.entry_id is null
    union all
...
r.link like concat(w.link,'%') or (r.website_id=10 and r.link like 'https://%.wikipedia.org/wiki/%') or (r.website_id in (16,19) and r.link like 'https://youtu.be/%') or (r.website_id in (16,19) and r.link like 'https://www.youtube.com/%') or (r.website_id=31 and r.link like 'https://%.itch.io/%'))
    union all
        select e.id,e.title,t.text,'** conflicting information about original publication' from entries e left join publicationtypes t on t.id = e.publicationtype_id where (e.publicationtype_id is null or e.publicationtype_id not in ('C','E','M')) and e.id in (select entry_id from compilations where is_original = 1) and e.id <= 15138
    union all
        select e.id,e.title,t.text,'conflicting information about original publication' from entries e left join publicationtypes t on t.id = e.publicationtype_id where (e.publicationtype_id is null or e.publicationtype_id not in ('C','E','M')) and e.id in (select entry_id from compilations where is_original = 1) and e.id > 15138
    union all
        select e.id,e.title,t.text,'** unidentified original publication' from entries e inner join publicationtypes t on t.id = e.publicationtype_id where e.publicationtype_id in ('C','E','M') and e.id not in (select entry_id from compilations where is_original = 1) and e.id <= 28700
    union all
        select e.id,e.title,t.text,'unidentified original publication' from entries e inner join publicationtypes t on t.id = e.publicationtype_id where e.publicationtype_id in ('C','E','M') and e.id not in (select entry_id from compilations where is_original = 1) and e.id > 28700
    union all
        select e.id,e.title,concat(b.name,' / ',d.name),'** redundant or conflicting information about original publisher' from entries e inner join publishers p on p.entry_id = e.id and p.release_seq = 0 and p.publisher_seq = 1 inner join labels b on b.id = p.label_id inner join compilations c on c.entry_id = e.id and c.is_original = 1 inner join entries k on k.id = c.compilation_id inner join publishers q on q.entry_id = k.id and q.release_seq = 0 and q.publisher_seq = 1 inner join labels d on d.id = q.label_id where e.id <= 27000
    union all
        select e.id,e.title,concat(b.name,' / ',d.name),'redundant or conflicting information about original publisher' from entries e inner join publishers p on p.entry_id = e.id and p.release_seq = 0 and p.publisher_seq = 1 inner join labels b on b.id = p.label_id inner join compilations c on c.entry_id = e.id and c.is_original = 1 inner join entries k on k.id = c.compilation_id inner join publishers q on q.entry_id = k.id and q.release_seq = 0 and q.publisher_seq = 1 inner join labels d on d.id = q.label_id where e.id > 27000
) as errors
