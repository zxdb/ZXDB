...
--    union all
--        select id,title,concat(rdt,' vs ',idt),'conflicting original publication date between entry and magazine' from (select e.id,e.title,concat(coalesce(r.release_year,'----'),'/',lpad(coalesce(r.release_month,'--'),2,'0'),'/',lpad(coalesce(r.release_day,'--'),2,'0')) as rdt,idt from entries e inner join releases r on r.entry_id = e.id and r.release_seq = 0 inner join (select min(concat(coalesce(i.date_year,'----'),'/',lpad(coalesce(i.date_month,'--'),2,'0'),'/',lpad(coalesce(i.date_day,'--'),2,'0'))) as idt,s.entry_id from issues i inner join magrefs s on s.issue_id = i.id and s.is_original=1 group by s.entry_id) as x on x.entry_id = e.id) as y where rdt <> idt
--    union all
--        select e.id,e.title,concat(coalesce(r.release_year,'-'),'/',coalesce(r.release_month,'-'),'/',coalesce(r.release_day,'-'),' vs ',coalesce(rk.release_year,'-'),'/',coalesce(rk.release_month,'-'),'/',coalesce(rk.release_day,'-')),'conflicting original publication date between entry and book' from entries e inner join releases r on r.entry_id = e.id and r.release_seq = 0 inner join booktypeins b on b.entry_id = e.id and b.is_original=1 inner join releases rk on rk.entry_id = b.book_id and rk.release_seq = 0 where coalesce(r.release_year,-1) <> coalesce(rk.release_year,-1) or coalesce(r.release_month,-1) <> coalesce(rk.release_month,-1) or coalesce(r.release_day,-1) <> coalesce(rk.release_day,-1)
--    union all
--        select e.id,e.title,concat(coalesce(r.release_year,'-'),'/',coalesce(r.release_month,'-'),'/',coalesce(r.release_day,'-'),' vs ',coalesce(rk.release_year,'-'),'/',coalesce(rk.release_month,'-'),'/',coalesce(rk.release_day,'-')),'++ conflicting original publication date between entry and container' from entries e inner join releases r on r.entry_id = e.id and r.release_seq = 0 inner join contents c on c.entry_id = e.id and c.is_original = 1 inner join releases rk on rk.entry_id = c.container_id and rk.release_seq = 0 where coalesce(r.release_year,-1) <> coalesce(rk.release_year,-1) or coalesce(r.release_month,-1) <> coalesce(rk.release_month,-1) or coalesce(r.release_day,-1) <> coalesce(rk.release_day,-1)
    union all
...
--    union all
--        select e.id,e.title,concat(coalesce(r.release_year,'-'),'/',coalesce(r.release_month,'-'),'/',coalesce(r.release_day,'-')),'** redundant or conflicting original publication date' from entries e inner join releases r on r.entry_id = e.id and r.release_seq = 0 where r.release_year is not null and (e.id in (select entry_id from contents where is_original=1) or e.id in (select entry_id from booktypeins where is_original=1) or e.id in (select entry_id from magrefs where is_original=1))
    union all
