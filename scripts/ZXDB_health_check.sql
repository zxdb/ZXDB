...
select * from (
    ...
    union all
        select id, title, mag_issue, 'title is apparently not a magazine issue' from (select e.id, e.title, i.number, replace(replace(replace(replace(m.name,'ACE (Advanced Computer Entertainment)','ACE'),'Sinclair User Club-DE','Sinclair User Club'),'Your Computer-ES','Your Computer'),'16-48 Magazine','16/48 Magazine Tape') as mag_name, concat(m.name,' #',coalesce(i.number,'?'),' ',coalesce(i.date_year,'?'),'/',coalesce(i.date_month,'?')) as mag_issue from entries e inner join issues i on i.id = e.issue_id inner join magazines m on i.magazine_id = m.id) as x where title not like '%DigiTape%' and not (title = mag_name or title = CONCAT(mag_name,' ',number) or title = CONCAT(mag_name,' 0',number) or title = CONCAT(mag_name,' No ',number) or title = CONCAT(mag_name,' No 0',number) or lower(title) = lower(CONCAT(mag_name,' Nr 0',number)) or lower(title) = lower(CONCAT(mag_name,' issue ',number)) or lower(title) = lower(CONCAT(mag_name,' issue 0',number)) or lower(title) = lower(CONCAT(mag_name,' issue 00',number)) or title like CONCAT(mag_name,' issue 0',number,':%') or title like CONCAT(mag_name,' issue ',number,':%') or title like CONCAT(mag_name,' issue 0',number,' -%'))
    union all
