...
select * from (
        select e.id as entry_id,e.title,d2.file_link as details,'compare and discard thumbnail if duplicated' as todo from entries e inner join downloads d1 on d1.entry_id = e.id and d1.release_seq = 0 inner join downloads d2 on d2.entry_id = e.id where (d1.file_link like '/pub/sinclair/books-pics/%' and d2.file_link like '/zxdb/sinclair/pics/books/%') or (d1.file_link like '/pub/sinclair/hardware-pics/%' and d2.file_link like '/zxdb/sinclair/pics/hw/%')
    union all
