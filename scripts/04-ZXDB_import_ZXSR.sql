...
drop table if exists tmp_caption;
...
insert into zxsr_reviews(id, intro_text, review_text, review_rating) (select r.review_id,
nullif(replace(x.intro_text,'\r',''),''),
replace(t.review_text,'\r',''),
nullif(replace(t.review_rating,'\r',''),'')
from zxsr.ssd_review r
inner join zxsr.ssd_review_text t on r.text_id = t.text_id
...
update zxsr_reviews set review_rating = SUBSTR(review_rating,1,CHAR_LENGTH(review_rating)-2) where review_rating like '%\n\n';
...
insert into tmp_review (id, page) (select review_id, trim(substring_index(replace(replace(lower(review_page),'.',','),'-',','),',',1)) from zxsr.ssd_review);
...
(1000738, 1049, 11, 3, 0, 'Wico Command Control'),      -- Wico Joysticks
(1000738, 1049, 11, 4, 1, 'Wico Ergostick'),
(1000730, 1049, 11, 1, 0, 'Turbo Pedal'),               -- Euromax Joysticks
(1000730, 1049, 11, 2, 1, 'Challenger Remote Control'),
(1000730, 1527, 62, 9, 0, 'Micro Handler'),             -- Euromax Joysticks
(1000730, 1527, 62, 7, 1, 'Pro Ace'),
...
inner join zxsr.ssd_review_score_header h on s.header_id = h.header_id
...
where h.header_text in ('Overall','Durability') and s.review_score = x.overall_score;
...
update magrefs r
inner join tmp_review t on r.id = t.magref_id
inner join zxsr.ssd_review z on t.id = z.review_id
inner join zxsr.ssd_review_text x on x.text_id = z.text_id
set r.award_id = if(z.award_id not in (999,41), z.award_id, null), r.review_id = z.review_id where 1=1;

-- Store review scores in ZXDB
insert into zxsr_scores(magref_id, score_seq, category, score, comments) (select t.magref_id, s.header_order, h.header_text, nullif(concat(coalesce(trim(s.review_score),''),coalesce(trim(s.score_suffix),'')),''), x.score_text
from tmp_review t
inner join zxsr.ssd_review_score s on s.review_id = t.id
inner join zxsr.ssd_review_score_header h on s.header_id = h.header_id
...
left join magrefs r on c.game_id = r.entry_id and z.zxdb_issue_id = r.issue_id and t.page = r.page and r.referencetype_id = 10 and r.score_group = ''
where r.id is null group by c.game_id, z.zxdb_issue_id, t.page order by c.game_id, z.zxdb_issue_id, t.page);
...
set r.review_id = z.review_id
where r.review_id is null;

-- Store compilation content's review scores in ZXDB
insert into zxsr_scores(magref_id, score_seq, category, score) (select r.id, if(c.review_header like '%(%',c.header_order,1), c.review_header, nullif(concat(coalesce(trim(c.review_score),''),coalesce(trim(c.score_suffix),'')),'')
from zxsr.ssd_review_score_compilations c
inner join zxsr.ssd_review z on c.review_id = z.review_id
inner join tmp_review t on z.review_id = t.id
...
insert into tmp_caption(magref_id, caption_seq, text, is_banner) (select t.magref_id, 0, replace(p.pic_text,'\r',''), p.is_banner
...
update tmp_caption set caption_seq=(select max(caption_seq)+1 from tmp_caption) where id in (select min(id) from tmp_caption where caption_seq=0 group by magref_id);
update tmp_caption set caption_seq=(select max(caption_seq)+1 from tmp_caption) where id in (select min(id) from tmp_caption where caption_seq=0 group by magref_id);
update tmp_caption set caption_seq=(select max(caption_seq)+1 from tmp_caption) where id in (select min(id) from tmp_caption where caption_seq=0 group by magref_id);
update tmp_caption set caption_seq=(select max(caption_seq)+1 from tmp_caption) where id in (select min(id) from tmp_caption where caption_seq=0 group by magref_id);
update tmp_caption set caption_seq=(select max(caption_seq)+1 from tmp_caption) where id in (select min(id) from tmp_caption where caption_seq=0 group by magref_id);
...
insert into zxsr_captions(magref_id, caption_seq, text, is_banner) (select magref_id, caption_seq, text, is_banner from tmp_caption);
